...
class RadialView {
  constructor(svg, data = [], dispatch) {
      ...
      .html(function (d) {
        return `
                    <p>
                        ...
                        ${
                          !d.preview_url
                            ? "<br><small>Song preview not available</small>"
                            : ""
                        }
                    </p>
                `;
    ...
    d3.select("body").on("click.rv", function () {
      ...
      var outsideDot = d3
        .selectAll(".song.active *")
        .filter(equalToEventTarget)
        .empty();
      var outsideButton = d3
        .selectAll(
          ".vis-control, .vis-control *, .dont-reset-selection, .dont-reset-selection *"
        )
        .filter(equalToEventTarget)
        .empty();
  ...
  onDataChanged(newData) {
    ...
    if (this.isSplitting) {
      if (
        new Set(this.data.map((d) => d[this.config.splitKey])).size !=
        newData.length
      ) {
  ...
  setConfig(config) {
    ...
    willBeRadial =
      willBeRadial ||
      (config.xMapping === undefined
        ? this.config.xMapping.isRadial
        : config.xMapping.isRadial);
    willBeRadial =
      willBeRadial ||
      (config.yMapping === undefined
        ? this.config.yMapping.isRadial
        : config.yMapping.isRadial);

    if (
      (config.isSplitting !== undefined &&
        config.isSplitting != this.config.isSplitting) ||
  ...
  initGrid() {
    ...
    for (let i = 0; i < this.SPLITS; i++) {
      let multiGridG = selectAllOrCreateIfNotExist(
        this.allGridsG,
        `g.grid-split#grid-split-${i}`
  ...
  redraw() {
    ...
    for (let i in this.CENTER_BY_NUM_SPLITS[this.SPLITS]) {
      let vinylG = selectAllOrCreateIfNotExist(
        vinylsLayer,
        `g.vinyl#vinyl-${i}`
      ).attr(
        "transform",
        `translate(${this.CENTER_BY_NUM_SPLITS[this.SPLITS][i].join(",")})`
      );

      if (this.useRadialScale()) {
        let vinylOuter = selectAllOrCreateIfNotExist(
          vinylG,
          "circle.vinyl-outer"
        )
        ...
        if (!this.config.isSplitting) {
          let vinylCenter = selectAllOrCreateIfNotExist(
            vinylGInner,
            "circle.vinyl-center"
          )
            .attr("r", r)
          ...
          let vinylCenterImage = selectAllOrCreateIfNotExist(
            vinylGInner,
            "circle.vinyl-center-image"
          )
            .attr("r", r)
  ...
  onHighlight(filterFunction) {
    ...
    d3.selectAll(".song").classed("fade", (d) => !this.highlight(d));
    // this.redraw();
  }
  ...
  scaleSelector(type) {
    ...
    else if (type == "log") return d3.scaleLog();
    else return d3.scaleLinear();
  }
  ...
  recomputeConsts() {
    ...
    let nGroups = d3
      ...
      .entries(this.filteredData).length;
    this.SPLITS = this.config.isSplitting ? Math.min(nGroups, MAX_SPLITS) : 1;

    this.MIN_RADIAL_DIST =
      this.SPLITS == 1
        ? Math.min(this.W, this.H) / 8
        : Math.min(this.W, this.H) / 24;
    this.MAX_RADIAL_DIST =
      this.SPLITS == 1
        ? Math.min(this.W, this.H) / 2 -
          Math.max(this.PADDING.x, this.PADDING.y)
        : Math.min(this.W, this.H) / 4 -
          Math.max(this.PADDING.x, this.PADDING.y);

    if (this.useRadialScale())
      this.SCALE_X = SCALE_ANGLE.domain(
        RADIAL_SCALE_DOMAIN_BY_KEY[this.config.xMapping.key]
      );
    else
      this.SCALE_X = this.scaleSelector(this.config.xMapping.scale)
        .domain(
          this.data.length == 0
            ? [0, 1]
            : [
                this.config.xMapping.minOverride ||
                  d3.min(this.data, (d) => d[this.config.xMapping.key]),
                this.config.xMapping.maxOverride ||
                  d3.max(this.data, (d) => d[this.config.xMapping.key]),
              ]
        )
        .range(
          this.SPLITS == 1
    ...
    this.SCALE_Y = this.scaleSelector(this.config.yMapping.scale)
      .domain(
        this.data.length == 0
          ? [0, 1]
          : [
              this.config.yMapping.minOverride ||
                d3.min(this.data, (d) => d[this.config.yMapping.key]),
              this.config.yMapping.maxOverride ||
                d3.max(this.data, (d) => d[this.config.yMapping.key]),
            ]
      )
      .range(
        this.useRadialScale()
    ...
    if (this.filteredData) {
      ...
      let dataByKey = d3
        ...
        .rollup((v) => ({
          sumArea: d3.sum(
            ...
            (d) =>
              Math.PI *
              tempScale(d[this.config.dotRadiusMapping]) *
              tempScale(d[this.config.dotRadiusMapping])
          ),
          count: v.length,
        }))
      ...
      let sizeDivisor = 800;
      this.SCALE_DOT_RADIUS = d3
        .scalePow()
        .exponent(0.5)
        .domain(d3.extent(this.data, (d) => d[this.config.dotRadiusMapping]))
        .range([
          2,
          (drawingArea / (this.SPLITS == 1 ? sizeDivisor : sizeDivisor * 2)) *
            Math.sqrt(1 / maxSumAreaSpoke),
        ]);
    }

    this.SCALE_DOT_COLOR = d3
      .scaleOrdinal(this.COLOR_SCHEME)
      .domain(this.collections.map((d) => d.id));
    this.SCALE_DOT_CHART_INDEX =
      this.SPLITS == 1
        ? (x) => 0
  ...
  initForce() {
    ...
    if (!this.force) {
      this.force = d3
        .forceSimulation(this.filteredData)
        ...
        .on("tick", function tick(e) {
          ...
          let similarityLinks = _this.lineLayer
            .selectAll("line.similarity-link")
    ...
    if (this.config.enableForce)
      this.force
        .force("collision")
        .radius(
          (d) => _this.SCALE_DOT_RADIUS(d[_this.config.dotRadiusMapping]) + 1.5
        )
        .iterations(7);
    else this.force.force("collision").radius(0);

    this.force
      .force("x")
      .x(function (d) {
        let center =
          _this.CENTER_BY_NUM_SPLITS[_this.SPLITS][
            _this.SCALE_DOT_CHART_INDEX(d[_this.config.splitKey])
          ];
  ...
  drawDataPoints() {
    ...
    var songG = dataPointsG
      .selectAll("g.song")
      .data(this.filteredData, (d) => d.id);

    var songGEnter = songG
      .enter()
      .append("g")
      .attr("class", "song")
      .attr("id", (d) => `song-${d.id}`)
      .call(
        d3
          ...
          .on("drag", function (d) {
            ...
            let dropTargetEl = document.elementFromPoint(
            ...
            console.log(
              dropTarget.size() > 0,
              dropTarget.size() > 0 && dropTarget.attr("id") == dropTargetId
            );
            d3.select("#drop-area").classed(
              "ready",
              dropTarget.size() > 0 && dropTarget.attr("id") == dropTargetId
            );
          })
          .on("end", function (d, i, m) {
            ...
            if (
              dropTarget.size() > 0 &&
              dropTarget.attr("id") == dropTargetId
            ) {
             // clearTutorial();
    ...
    var songGEnterInner = songGEnter
      .append("g")
      .attr("class", (d) => `song-inner rotate-anim rotate-${d.time_signature}`)
      .style(
        "animation-duration",
        (d) => `${(60 / d.tempo) * d.time_signature}s`
      );
   ...
   /* var dotHoverArea = songGEnterInner
      .append("circle")
    ...
    dotHoverArea
      ...
      .on("mousedown", function () {
        callTutorial(
          "#drop-area",
          "Drag and drop it here to see more details",
          "n"
        );
      })
    ...
    songG
      ...
      .attr(
        "width",
        (d) => 2 * this.SCALE_DOT_RADIUS(d[this.config.dotRadiusMapping])
      )
      .attr(
        "height",
        (d) => 2 * this.SCALE_DOT_RADIUS(d[this.config.dotRadiusMapping])
    ...
    songG.merge(songGEnter).classed("fade", (d) => !this.highlight(d));
    ...
    songG
      ...
      .style("fill", (d) =>
        this.config.showAlbumArt
          ? `url(#image${d.id})`
          : this.SCALE_DOT_COLOR(d[this.config.splitKey])
      )
      .transition()
    ...
    songG
     ...
     .selectAll("polygon.dot")
      .attr("points", function (d) {
        ...
        var theta_offset_radial = _this.useRadialScale()
          ? SCALE_ANGLE(_this.getKeyFromKeyId(d.key, d.mode))
          : -Math.PI / 2;
        var theta_offset =
          (2 * Math.PI) / d.time_signature / 2 + Math.PI + theta_offset_radial;

        var size = _this.SCALE_DOT_RADIUS(d[_this.config.dotRadiusMapping]);
        var targetArea = Math.PI * size * size;
        var axesLength = Math.sqrt(
          targetArea / (n * Math.sin(Math.PI / n) * Math.cos(Math.PI / n))
        );
      ...
      return function (d, i, m) {
        ...
        if (!_this.selectionLocked) {
          ...
          _this.dispatch.call("highlight", this, function (k) {
            ...
            if (
              searchBox.classed("active") &&
              searchBox.node().value.trim().length
            ) {
              if (k.name)
                isInFilter =
                  k.name
                    .toLowerCase()
                    .indexOf(searchBox.node().value.trim().toLowerCase()) >= 0;
              else isInFilter = false;
            }
            // let isSimilarSong = _this.similarSongsToSelection.filter(x => x.song.id == k.id).length > 0;
  ...
  selectSong(d, elem, k = 5) {
    ...
    let _this = this;
    let activeBrushes = d3.selectAll(".brush .selection").filter(function () {
    ...
    d3.select(`.vinyl-center-image`).classed("hide", false);
    d3.select("#image_center_0 image").attr("xlink:href", d.images[0].url);
  }
  ...
  resetSelection() {
        ...
        if (d3.select("input#search-highlight").classed("active")) {
          ...
          return (
            k.name
              .toLowerCase()
              .indexOf(
                d3.select("input#search-highlight").node().value.toLowerCase()
              ) >= 0
          );
        } else {
          return true;
    ...
    d3.select(`.vinyl-center-image`).classed("hide", true);
  }
  ...
  dataToXy(d, distanceOverride, isRadial = true) {
    if (isRadial) {
      ...
      let distance =
        distanceOverride || this.SCALE_Y(d[this.config.yMapping.key]);
